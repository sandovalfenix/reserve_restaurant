        ...
        $.get("/reserver/read", function(dataEvent) {
                    ...
                    o.popover({
                        ...
                        html: 
                            ...
                            content:'<div class="m-scheduler__event-heading pt-3">'+i.get(0).outerHTML+
                                    ...
                                    '<ul class="list-group">'+
                                      '<li class="list-group-item"><i class="fas fa-clipboard-check"></i> Solicitud: <b>'+t.request+'</b></li>'+
                                      ...
                                      '<li class="list-group-item"><i class="fas fa-envelope"></i> Correo: <a href="mailto:'+t.email+'"><b>'+t.email+'</b></a></li>'+
                                       ...
                                       '<li class="list-group-item"><i class="fas fa-glass-cheers"></i> Celebración: <b>'+t.description+'</b></li>'+
                                    '</ul>'+
                                    '</div>', 
            ...
            e(document).on("click", function(t) {
                ...
                $('a[data-idEditModal]').on('click', function() {
                    var idReserver = $(this).attr('data-idEditModal');
                    $.ajax({
                        url: '/reserver/editForm/'+idReserver,
                        type: 'POST',
                    })
                    .done(function(data) {                 
                        $('#modalReserver').html(data);                    
                        $('form').attr('action','/reserver/update/'+idReserver);
                ...
                $('a[data-idRemoveModal]').on('click', function() {
                    var idReserver = $(this).attr('data-idRemoveModal');
                    $('form').attr('action', '/reserver/delete/'+idReserver);
                });
            })
        });
    })
